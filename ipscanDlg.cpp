...
void CIpscanDlg::OnScanSavetotxt() 
{
	...
	CFileDialog fd(FALSE,"txt",NULL,OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,"Text files (*.txt)|*.txt|All files (*.*)|*.*||",d);
	if (fd.DoModal()==IDOK) {
		FILE *f;
		f = fopen(fd.GetPathName(),"wt");
		if (!f) {
			MessageBox("Cannot open file for writing",NULL,MB_OK | MB_ICONHAND);
			return;
		}
		fprintf(f,"This file was generated by Angry IP Scanner\nVisit http://www.aziber.com/ for the latest version\n\n");
		
		int i,j;
		int ws[C_COLUMNS];
		CString tmp;
		for (i=0; i<C_COLUMNS; i++) {
			ws[i] = m_list.GetColumnWidth(i) / 8 + 2;
		...
		for (i=0; i<m_list.GetItemCount(); i++) {
			for (j=0; j<C_COLUMNS; j++) {
				fprintf(f,"%-*s",ws[j],m_list.GetItemText(i,j));
			}
			fprintf(f,"\n");
		}
		fclose(f);
...
{
	ShellExecute(0,NULL,"mailto:angryziber@mail.com",NULL,NULL,SW_SHOWNORMAL);	
}

void CAboutDlg::OnGohttp() 
{
	ShellExecute(0,NULL,"http://www.angryipscanner.com/",NULL,NULL,SW_SHOWNORMAL);
}
...
void CIpscanDlg::OnScanSaveselection() 
{
	...
	if (m_list.GetSelectedCount()==0) {
		MessageBox("You must select items first",NULL,MB_OK | MB_ICONHAND);
		return;
	}
	CFileDialog fd(FALSE,"txt",NULL,OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,"Text files (*.txt)|*.txt|All files (*.*)|*.*||",d);
	if (fd.DoModal()==IDOK) {
		...
		fprintf(f,"\n\n");
		for (i=0; i<m_list.GetItemCount(); i++) {
			it.mask = LVIF_STATE; it.stateMask = LVIS_SELECTED;
			it.iItem = i; it.iSubItem = CL_IP; 
			m_list.GetItem(&it); 
...
{
	// TODO: Add your command handler code here
	app->WriteProfileString("","URL","http://www.angryipscanner.com/");
...
{
	// TODO: Add your command handler code here
	ShellExecute(0,NULL,"http://www.aziber.com/",NULL,NULL,SW_SHOWNORMAL);
...
{
	MessageBox("You must select IP first","Error",MB_OK | MB_ICONERROR);
}
