...
public class PortTextFetcherPrefs extends AbstractModalDialog implements FetcherPrefs {
	...
	protected void populateShell() {
		...
		var sendLabel = new Label(shell, SWT.NONE);
		...
		var matchLabel = new Label(shell, SWT.NONE);
		...
		var replaceLabel = new Label(shell, SWT.NONE);
	...
	void savePreferences() {
		var prefs = fetcher.getPreferences();

		var text = toRealText(textToSend.getText());
	...
	static String toRealText(String s) {
		...
		if (s.contains("\\x")) {
			var t = new StringBuilder();
			var m = Pattern.compile("\\\\x(\\d{2})").matcher(s);
			while (m.find()) {
